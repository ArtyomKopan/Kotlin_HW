...
class Vector<T : ArithmeticAvailable<T>>(val coordinates: List<T>) {
    ...
    operator fun plus(increment: Vector<T>): Vector<T> {
        ...
        return Vector((0 until this.dimension).map { coordinates[it] + increment.coordinates[it] }.toList())
    }

    operator fun minus(decrement: Vector<T>): Vector<T> {
        require(this.dimension == decrement.dimension) { "Длины векторов не совпадают!" }
        return Vector((0 until this.dimension).map { this.coordinates[it] - decrement.coordinates[it] }.toList())
    }

    operator fun times(factor: Vector<T>): T {
